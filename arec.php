...
$sql = "SELECT lname,mobnum,age,email,blood_group,BUNIT,refhosp,l_recieved FROM recepientdetails";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["lname"]. "</td><td>" . $row["mobnum"] . "</td><td>" .$row["age"] . "</td><td>".$row["blood_group"] . "</td><td>".$row["BUNIT"] . "</td><td>".$row["refhosp"] ."</td><td>". $row["l_recieved"]. "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
